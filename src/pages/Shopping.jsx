...
function buildCopyAllText(categories, manualItems, mealObjects, quantities, checked) {
...
function buildByRecipeText(categories, manualItems, mealObjects, quantities, checked) {
  ...
  mealObjects.forEach(meal => {
    ...
    lines.push(meal.name)
    const enriched = Array.isArray(quantities[meal.id]) ? quantities[meal.id] : (meal.ingredients || [])
    enriched.filter(ing => !checked.has(ing)).forEach(ing => lines.push(`  • ${ing}`))
  })
...
function ShoppingShareModal({ categories, manualItems, mealObjects, quantities, checked, onClose }) {
  ...
  const [format, setFormat] = useState('copyall') // 'copyall' | 'byrecipe'

  const copyAllText = useMemo(
    () => buildCopyAllText(categories, manualItems, mealObjects, quantities, checked),
  ...
  const text = format === 'copyall' ? copyAllText : byRecipeText
    ...
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal modal-send slide-up" onClick={e => e.stopPropagation()}>
        ...
        <div className="send-format-tabs">
          <button
            className={`send-format-tab ${format === 'copyall' ? 'active' : ''}`}
            onClick={() => { setFormat('copyall'); setCopied(false) }}
          >
            Copy All Format
          </button>
          <button
